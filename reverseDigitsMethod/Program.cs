...
namespace reverseDigitsMethod
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            string decNum = (Console.ReadLine()); //input number as string
        ...
        {
            char[] letters = toReverse.ToCharArray(); //takes number (as string) and converts to char array
            Array.Reverse(letters); //reverses the char array
            string reversedString = new string(letters); //makes a new string out of the char array
            decimal reversedToDecimal = decimal.Parse(reversedString); //parses the new string to a decimal
            return reversedToDecimal; //returns the decimal 
        }
    }
}
